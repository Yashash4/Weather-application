...
async function fetchHourlyForecast(city) {
  try {
    ...
    const data = await res.json();
    if (data.cod !== "200") { alert("Hourly forecast data not available!"); return; }
    for (let i = 0; i < 12; i++) {
...
async function fetchFutureForecast(city) {
  try {
    const res = await fetch(`${forecastURL}?q=${city}&appid=${API_KEY}&units=metric`);
    const data = await res.json();
    if (data.cod !== "200") { alert("Future forecast data not available!"); return; }
    const dailyForecasts = {};
    data.list.forEach(entry => {
      const [date, time] = entry.dt_txt.split(" ");
      if (time === "12:00:00") { dailyForecasts[date] = entry; }
    });
    ...
    dates.forEach((date, index) => {
      ...
      let dayElem = document.querySelector(`#day${index + 1} .day`);
      if (!dayElem) {
        dayElem = document.createElement("div");
        dayElem.classList.add("day");
        document.querySelector(`#day${index + 1} .other`).insertBefore(dayElem, document.querySelector(`#day${index + 1} .other`).firstChild);
      }
      dayElem.textContent = dayName;
      document.getElementById(`day${index + 1}-temp`).textContent = `Temp: ${forecast.main.temp.toFixed(1)}°C`;
      document.getElementById(`day${index + 1}-humid`).textContent = `Humidity: ${forecast.main.humidity}%`;
      document.getElementById(`day${index + 1}-cond`).textContent = `Condition: ${forecast.weather[0].description}`;
